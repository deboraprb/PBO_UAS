...
class User:
    ...
    def __init__(self, id, name, email, phone, role, created_at=None):
        self.id = id
        self.name = name
        self.email = email
        self.phone = phone
...
def get_reservations():
    if session.get("role") == "admin":
        ...
        rows = reservation_repo.get_by_user(session["user_id"])
...
def cancel_reservation(id):
    ...
    result = reservation_service.cancel_reservation(id, session["user_id"], is_admin)
    return jsonify(result), 200 if result["success"] else 400
